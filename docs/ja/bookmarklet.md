...
4. B 系列の種類を選び、必要に応じて `-lv 13 -diff mas` のような絞り込みを入力して **Generate** を押します。
5. 生成が完了すると、ページ内のオーバーレイで画像を確認できます。同じブラウザタブ内では、ページ更新後に再度ブックマークを開いた場合も前回のオーバーレイを復元し、取得済みの成績データを再利用します。
