...
4. 必要に応じて `settings` ページで生成した Import Token を入力します。保存すると、このブラウザに保持されます。手動アップロードに使え、ローカルにアップロード記録がない場合は起動時に一度アップロードします。
...
- Import Token を保存しても、Generate のたびに強制アップロードはしません。必要なときは Upload ボタンを使います。
